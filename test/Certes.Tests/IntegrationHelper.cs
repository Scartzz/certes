...
public static class IntegrationHelper
{
    ...
    {
        using var resp = await http.Value.PutAsync($"http://certes-ci.dymetis.com/dns-01/{algo}", new StringContent(JsonSerializer.Serialize(tokens), Encoding.UTF8, "application/json"));
    ...
    public static async Task<IOrderContext> AuthorizeHttp(AcmeContext ctx, IList<string> hosts)
    {
        for (var i = 0; i < 10; ++i)
        {
            ...
            var authrizations = (await orderCtx.Authorizations()).ToList();
