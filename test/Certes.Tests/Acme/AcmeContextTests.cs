...
public class AcmeContextTests
{
    ...
    public async Task CanChangeKey()
    {
        ...
        httpMock.Setup(
                m => m.Post<object, Account>(MockDirectoryV2.KeyChange, It.IsAny<object>()))
            .ReturnsAsync(new AcmeHttpResponse<Account>(
                accountLoc, new Account
                {
                    Status = AccountStatus.Valid
    ...
    public async Task CanRevokeCertByPrivateKey()
    {
        ...
        httpClientMock.Setup(m => m.Post<object, string>(Helper.MockDirectoryV2.RevokeCert, It.IsAny<object>()))
