...
public class OrderFinalizeCommandTests
{
    ...
    public async Task CanProcessCommand()
    {
        ...
        var ret = JsonSerializer.Deserialize<Dictionary<string, JsonElement>>(stdOutput.ToString());
        var privateKeyBytes = Convert.FromBase64String($"{ret["privateKey"].GetString()}");
        ...
        Assert.True(errOutput.Length == 0, errOutput.ToString());
        ret = JsonSerializer.Deserialize<Dictionary<string, JsonElement>>(stdOutput.ToString());
        Assert.Equal(
            JsonSerializer.Serialize(new
            {
                location = orderLoc,
            ...
            JsonSerializer.Serialize(ret, JsonSettings));
