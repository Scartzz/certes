...
public class CertificatePemCommandTests
{
    ...
    public async Task CanProcessCommand()
    {
        ...
        var ret = JsonSerializer.Deserialize<object>(stdOutput.ToString(), JsonUtil.CreateJsonSettings());
        Assert.Equal(
            ...
            JsonSerializer.Serialize(ret, JsonUtil.CreateJsonSettings()));
        ...
        Assert.True(errOutput.Length == 0, errOutput.ToString());
        ret = JsonSerializer.Deserialize<object>(stdOutput.ToString());
        Assert.Equal(
            JsonSerializer.Serialize(new
            {
                location = certLoc,
