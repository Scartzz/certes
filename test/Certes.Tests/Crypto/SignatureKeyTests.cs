...
public class SignatureKeyTests
{
    ...
    private void CanExportKey(KeyAlgorithm signatureAlgorithm)
    {
        ...
        Assert.Equal(
            JsonSerializer.Serialize(key.JsonWebKey),
            JsonSerializer.Serialize(exported.JsonWebKey));
    ...
    private void EnsurePropertySerializationOrder()
    {
        ...
        var k = KeyFactory.FromPem(Keys.ES256Key_Alt1);
        var ecKey = (EcJsonWebKey)k.JsonWebKey;
        var ecKeyJson = JsonSerializer.Serialize(ecKey, JsonUtil.CreateJsonSettings());
        ...
        var rsaKeyJson = JsonSerializer.Serialize(rsaKey, JsonUtil.CreateJsonSettings());
