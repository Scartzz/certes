...
internal class OrderContext : EntityContext<Order>, IOrderContext
{
    ...
    public async Task<Order> Finalize(byte[] csr)
    {
        ...
        var resp = await Context.HttpClient.Post<Order.Payload, Order>(Context, order.Finalize, payload, true);
    ...
    {
        var order = await Resource();
        var resp = await Context.HttpClient.Post<object, string>(Context, order.Certificate, null, false);
        ...
        {
            resp = await Context.HttpClient.Post<object, string>(Context, alternate, null, false);
