...
public class AcmeHttpClient : IAcmeHttpClient
{
    ...
    private readonly static JsonSerializerOptions jsonSettings = JsonUtil.CreateJsonSettings();
    ...
    /// <returns></returns>
    public async Task<AcmeHttpResponse<TResponse>> Post<TRequest, TResponse>(Uri uri, TRequest payload)
    {
        var payloadJson = JsonSerializer.Serialize<TRequest>(payload, jsonSettings);
        ...
        AddUserAgentHeader(msg);
        using var response = await Http.SendAsync(msg);
        return await ProcessResponse<TResponse>(response, uri);
    }
    ...
    private async Task<AcmeHttpResponse<T>> ProcessResponse<T>(HttpResponseMessage response, Uri requestedUri)
    {
        ...
        else
        {
            ...
            {
                var json = await response.Content.ReadAsStringAsync();
                error = JsonSerializer.Deserialize<AcmeError>(json);
            }
            else
            {
